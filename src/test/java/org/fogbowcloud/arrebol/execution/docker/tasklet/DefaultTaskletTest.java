...
public class DefaultTaskletTest {
  ...
  public void testSuccessfulExecution() throws Exception {
    ...
    Mockito.when(taskletHelper.getExitCodes(task.getId(), task.getTaskSpec().getCommands().size()))
        .thenReturn(MOCK_EC_ARRAY);
  ...
  public void testExceptionWhileSendTaskScriptExecutor() throws Exception {
    TaskletHelper taskletHelper = Mockito.mock(TaskletHelper.class);
    Mockito.doThrow(
            new Exception("Cannot send task script executor to worker=" + MOCK_CONTAINER_NAME))
        .when(taskletHelper)
        .sendTaskScriptExecutor(loadTaskScriptExecutor());
  ...
  public void testExceptionWhileSendTaskScript() throws Exception {
    TaskletHelper taskletHelper = Mockito.mock(TaskletHelper.class);
    Mockito.doThrow(
            new Exception("Error while trying to send command [echo Hello World] exit code=1"))
        .when(taskletHelper)
        .sendTaskScriptExecutor(loadTaskScriptExecutor());
