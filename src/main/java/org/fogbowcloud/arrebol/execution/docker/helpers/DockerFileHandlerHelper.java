...
public class DockerFileHandlerHelper {
  ...
  public Integer writeToFile(String containerId, String content, String filePath) {
    ...
    try {
      Integer exitCode =
          this.dockerCommandExecutor.executeCommand(apiAddress, containerId, command).getExitCode();
      return exitCode;
    } catch (Exception e) {
  ...
  public String readFile(String containerId, String filePath) {
    ...
    try {
      String fileContent =
          this.dockerCommandExecutor.executeCommandWithStout(apiAddress, containerId, command);
      return fileContent;
    } catch (Exception e) {
      throw new RuntimeException(
          "Error while read file [" + filePath + "] from container [" + containerId + "]");
    }
  }
}
