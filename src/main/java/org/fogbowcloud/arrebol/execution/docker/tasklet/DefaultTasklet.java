...
public class DefaultTasklet implements Tasklet {
  ...
  public TaskExecutionResult execute(Task task) {
    ...
    try {
      ...
      taskExecutionResult =
          new TaskExecutionResult(getTaskResult(commands), getExitCodes(commands));
      LOGGER.debug(
          "Result of task [" + task.getId() + "]: " + taskExecutionResult.getResult().toString());
    }
  ...
  private void setupContainerEnvironment(String taskId, List<Command> commands) throws Exception {
    ...
    LOGGER.debug(
        "Starting to write commands [len="
            + commands.size()
            + "] of task "
            + taskId
            + " to .ts file.");
