...
public class DefaultDockerContainerResource implements DockerContainerResource {
  ...
  @Transient private DockerContainerRequestHelper dockerContainerRequestHelper;
  @Transient private DockerImageRequestHelper dockerImageRequestHelper;

  /** @param resourceId Sets the name of the container, is an identifier. */
  public DefaultDockerContainerResource(
      String resourceId,
      DockerContainerRequestHelper dockerContainerRequestHelper,
  ...
  public void start(ContainerSpecification containerSpecification)
      throws DockerStartException, DockerCreateContainerException, UnsupportedEncodingException {
    if (isStarted()) {
  ...
  private Map<String, String> getDockerContainerRequirements(Map<String, String> taskRequirements) {
    ...
    if (Objects.nonNull(taskRequirements)) {
      String dockerRequirements = taskRequirements.get(DockerConstants.DOCKER_REQUIREMENTS_KEY);
        ...
        for (String requirement : requirements) {
          ...
          switch (key) {
            case DockerConstants.DOCKER_MEMORY_KEY:
              ...
              LOGGER.info(
                  "Added requirement ["
                      + DockerConstants.JSON_KEY_MEMORY
                      + "] with value ["
                      + value
                      + "] to container ["
                      + this.resourceId
            ...
            case DockerConstants.DOCKER_CPU_WEIGHT_KEY:
              ...
              LOGGER.info(
                  "Added requirement ["
                      + DockerConstants.JSON_KEY_CPU_SHARES
                      + "] with value ["
                      + value
                      + "] to container ["
                      + this.resourceId
  ...
  public void stop() throws DockerRemoveContainerException {
    if (!isStarted()) {
      throw new DockerRemoveContainerException(
          "Container[" + this.resourceId + "] was already stopped");
    }
