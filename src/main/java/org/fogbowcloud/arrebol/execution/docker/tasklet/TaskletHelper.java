...
public class TaskletHelper {
  ...
  public void runTaskScriptExecutor(String taskId) throws Exception {
    ...
    String command =
        String.format(
            RUN_TS_EXECUTOR_PATTERN, TASK_SCRIPT_EXECUTOR_FILE_PATH, taskScriptExecutorFilePath);
  ...
  public void sendTaskScript(String taskId, List<Command> commands) throws Exception {
    ...
    try {
      int exitCode =
          dockerFileHandlerHelper.writeToFile(containerId, taskScriptContent, taskScriptFilePath);
      if (exitCode != 0) {
        throw new Exception(
