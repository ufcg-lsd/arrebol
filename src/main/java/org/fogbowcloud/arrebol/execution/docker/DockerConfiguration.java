...
public class DockerConfiguration {
  ...
  public DockerConfiguration(Configuration configuration) throws Exception {
    ...
    Property<Double> workerPoolSize =
        configuration.getProperty(DockerConstants.JSON_WORKER_PULL_SIZE_KEY);
    Property<String> imageId = configuration.getProperty(DockerConstants.JSON_IMAGE_ID_KEY);
    Property<List<String>> resourceAddresses =
        configuration.getProperty(DockerConstants.JSON_RESOURCE_ADDRESSES_KEY);
  ...
  private void checkDockerConfigurationProperties(Configuration configuration) throws Exception {
    ...
    String resourceAddressesMsg =
        "Resource addresses configuration property wrong or missing." + verifyMsg;
    String workerPoolSizeMsg =
        "Worker pool size configuration property wrong or missing." + verifyMsg;
    ...
    if (imageId.getValue() == null
        || imageId.getValue().trim().isEmpty()
        || imageId.getValue().contains(":")) {
