...
public class DefaultK8sClient implements K8sClient {
    ...
    @Override
    public K8sJob createJob(String name, String imageId, String memoryRequest, String cpuRequest, String command) throws ApiException {
    	List<String> commandList = buildCommands(command);
    	K8sContainer podContainer = new K8sContainer()
    			.name(name)
                .image(imageId)
                .command(commandList);
    		
    	
        K8sPodSpec podSpec = new K8sPodSpec()
                .restartPolicy("Never")
        ...
        K8sJob job =  new K8sJob()
                .apiVersion("batch/v1")
                .kind("Job")
                .metadata(
                        new K8sObjectMeta().name(name))
                .spec(new K8sJobSpec()
                        .template(new K8sPodTemplateSpec()
		...
		final String jobEndpoint = this.batchApi.getApiClient().getBasePath() + "/apis/batch/v1/namespaces/default/jobs";
	...
	public K8sJob getJob(String name) throws ApiException {
		...
		final String jobEndpoint = this.batchApi.getApiClient().getBasePath() + "/apis/batch/v1/namespaces/default/jobs/" + name;
		K8sJob resultJob = null;
