...
public class JobProcessorManager {
  ...
  public List<DefaultJobProcessorDTO> getJobProcessors() {
    ...
    for (JobProcessor jobProcessor : queues.values()) {
      DefaultJobProcessorDTO defaultJobProcessorDTO =
          new DefaultJobProcessorDTO((DefaultJobProcessor) jobProcessor);
  ...
  public void startJobProcessor(String queueId) {
    ...
    this.jobProcessorDatabaseCommitter.schedule(
        new TimerTask() {
          public void run() {
        ...
        COMMIT_PERIOD_MILLIS,
        COMMIT_PERIOD_MILLIS);

    this.jobStateMonitor.schedule(
        new TimerTask() {
          public void run() {
            ...
            try {
              ...
              LOGGER.error(
                  "Error while updating job states from queue ["
                      + queueId
                      + "]: "
                      + e.getMessage());
            }
  ...
  private void updateJobState(Job job) {
    ...
    if (!(jobState.equals(JobState.FAILED) || jobState.equals(JobState.FINISHED))) {
      ...
      } else if (all(job.getTasks(), TaskState.FINISHED.getValue() + TaskState.FAILED.getValue())) {
