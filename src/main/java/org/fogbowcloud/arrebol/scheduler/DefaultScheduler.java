...
public class DefaultScheduler implements Runnable {
  ...
  public DefaultScheduler(TaskQueue queue, WorkerPool pool, SchedulerPolicy policy) {
    ...
    // a notification is received (notifyAddedJob or notifyAvailableResource). Failures are just
    // ignored.
    // There are plenty of possible improvements: e.g waiting a batch of notifications before
    // deciding ...
    // to improve utilization, we also add the notification to a queues that is consume by a planner
    // thread
